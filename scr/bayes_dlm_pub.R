...
pmNMMAPS <- function () {
  ...
  df = load.data()
  adds = add.lag(df, "pm25", "FIPS", lags=0:7)
  df = cbind(df, adds[[1]])
  return(df)
}
...
# updateBeta
# polloffset = rep(0, nrow(data))
# polloffset
# oldbet = NULL

# fitPartialMod
# data = cbind(data, polloffset = polloffset)
# pollutant = NULL
# nonpollutant = TRUE
# offset = "polloffset"
...
bayesDLM <- function(citydata, pollutant = "pm25", nLags = 3, sigma = 0.004, its = 10) {
  time1 <- proc.time()[3]
...
updateBeta <- function(data, polloffset, oldbet = NULL) {
  ...
  print(fit)
  print("updateBeta 222")
  print(cbind(data, polloffset))
...
# checked 
formPartialMod <- function(cause = "cases", pollutant = NULL, nLags = 0, nonpollutant = TRUE,
                           df.date, df.tmmx, df.sph, offset = NULL) {
  
  stopifnot(nonpollutant == is.null(pollutant), !is.null(offset))       #### what doe the offset do ???? 
  form.str <- paste(cause, "~")
  
  ## formula for non-pollutant model
  if (nonpollutant) {
    covariates.f <- "dayofweek + log(population)"
    
    ## Smooth functions of temperature and dew points (w/running means) ???
    weather.f <- paste(c(paste("ns(tmmx,", df.tmmx, ")"),
                         paste("ns(sph,", df.sph, ")")), collapse = "+")
    
    ## Smooth function(s) of time 
    time.f <- paste(c(paste("ns(date,", df.date, ")")), collapse = "+")
    
    form.str <- paste(form.str, paste(c(covariates.f, time.f, weather.f), collapse = "+"))
  }
  
  ## formula for pollutant model
  if (!is.null(pollutant)) {
    form.str <- paste(form.str, "-1 + ", pollutant) ############ why -1???????????????
    if (nLags > 0) {
      lagpoll.f <- paste(pollutant, ".l", 1:nLags, collapse = " + ", sep = "")
...
fitPartialMod <- function(data, pollutant = "pm25", nLags = 3, cause = "cases",
                          nonpollutant = FALSE, df.date = 8, df.tmmx = 2, df.sph = 2, 
                          offset = NULL, extractors = NULL) {
  ## Argument checking
  stopifnot(is.character(pollutant) | is.null(pollutant),
            is.character(cause), length(cause) == 1)
  print("modelFormula before")
  modelFormula <- formPartialMod(cause = cause, pollutant = pollutant, nLags = nLags,
                                 nonpollutant = nonpollutant, df.date = df.date,
                                 df.tmmx = df.tmmx, df.sph = df.sph, offset = offset)
  print("modelFormula")
  print(modelFormula)
  ## Fit the model!
