...
monitor_data <- rbindlist(lapply(list.files(READPATH, full.names = T, pattern='csv'), fread))
names(monitor_data) <- airnow_names

monitor_data[, day := date(ymd_hm(datetime))]
monitor_data <- monitor_data[, .(pm25 = mean(concentration, na.rm = T), lat = max(lat), lon = max(lon)), 
                     by = c("full_aqs_id", "day")]
...
fwrite(st_drop_geometry(monitor_data), paste0(WRITEPATH, "daily_pm_2020.csv"), row.names=F)
